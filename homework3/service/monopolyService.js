...
// new join query
router.get('/playerscores', readPlayerScores);
...
// new function to read all player scores for all games
function readPlayerScores(req, res, next) {
    db.many('SELECT Game.ID, Player.name, PlayerGame.score FROM PlayerGame INNER JOIN Player ON PlayerGame.playerID=Player.ID INNER JOIN Game ON PlayerGame.gameID=Game.ID', req.params)
    .then(data => {
        returnDataOr404(res, data);
    })
    .catch(err => {
